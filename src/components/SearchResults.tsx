  ...
  const results = useMemo(() => {
    return allOffers.filter((offer) => {
      const matchTerm =
        ...
        offer.content.toLowerCase().includes(term) ||
        offer.description.toLowerCase().includes(term) ||
        offer.fullDescription.toLowerCase().includes(term) ||
        offer.date.includes(term) ||
        offer.time.includes(term) ||
        offer.location.toLowerCase().includes(term) ||
        offer.district.toLowerCase().includes(term);
      
      const matchCategory = !category || offer.category.toLowerCase().includes(category);
      const matchDistrict = !district || offer.district.toLowerCase() === district;
      
      return matchTerm && matchCategory && matchDistrict;
    });
  }, [term, category, district]);
    ...
    <div className="min-h-screen bg-gray-900 pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-3xl font-bold text-white mb-2">Suchergebnisse</h2>
            {term && (
              <p className="text-gray-300">
                Suche nach: <span className="text-orange-500 font-medium">"{term}"</span>
              </p>
            )}
            {category && (
              <p className="text-gray-300">
                Kategorie: <span className="text-orange-500 font-medium">{category}</span>
              </p>
            )}
            {district && (
              <p className="text-gray-300">
        ...
        {results.length === 0 ? (
          <div className="bg-gray-800 p-8 rounded-xl text-center border border-gray-700">
            <p className="text-gray-300 mb-4">Keine Ergebnisse gefunden.</p>
            <button
              onClick={() => navigate("/")}
              className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2 rounded-lg font-medium transition-all"
            >
